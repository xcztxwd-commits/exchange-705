...
onMounted(async () => {
  ...
  const chartHeight = containerHeight // 不再强制限制高度为 Math.max(300)，让它自适应父容器
  ...
  try {
    ...
    chart.value.createTechnicalIndicator({
      ...
      styles: {
        line: [
          {
            color: '#F6DC50', // MA5 Yellow
            size: 1.5
          },
          {
            color: '#5094F6', // MA10 Blue
            size: 1.5
          },
          {
            color: '#F650D5', // MA30 Purple
            size: 1.5
          }
        ]
      }
  ...
  const stopWatch2 = watch(
    () => {
      const interval = props.interval || '5m'
      const klines = marketStore.klineDataMap[`${props.symbol}_${interval}`] || []
      return klines
    },
    (klines) => {
      if (!isMounted.value || !chart.value) return
...
async function loadHistoricalKlines() {
  try {
    ...
    // 先检查store中是否有数据
    let klines = marketStore.klineDataMap[`${props.symbol}_${interval}`] || []
    ...
    if (!klines || klines.length < 50) {
      ...
      klines = marketStore.klineDataMap[`${props.symbol}_${interval}`] || []
...
function updateKlines(klines: KlineData[]) {
  ...
  try {
    ...
    if (validKlines.length >= 30) {
      const last30 = validKlines.slice(-30)
      const ma30 = last30.reduce((sum, k) => sum + Number(k.close), 0) / 30
      ma30Value.value = ma30.toFixed(5)
    }
...
<template>
  <div class="chart-wrapper h-full flex flex-col relative">
    <!-- MA指标显示 -->
    <div class="ma-indicators shrink-0 flex justify-between items-center w-full">
      <div class="flex gap-2 sm:gap-4">
        <span class="ma-item ma5">MA5: {{ ma5Value || '--' }}</span>
        <span class="ma-item ma10">MA10: {{ ma10Value || '--' }}</span>
        <span class="ma-item ma30">MA30: {{ ma30Value || '--' }}</span>
      </div>
      <div class="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 font-medium whitespace-nowrap ml-2">
        Vol(24H): {{ volume24h || '--' }}
      </div>
    </div>
    
    <!-- K线图 -->
    <div ref="chartContainer" class="kline-chart flex-1 w-full min-h-0"></div>
  </div>
</template>
...
.ma-indicators {
  ...
  font-size: 10px;
  background: #ffffff;
  @apply dark:bg-[#131722];
  border-bottom: 1px solid #f3f4f6;
  @apply dark:border-[#2b3139];
}
