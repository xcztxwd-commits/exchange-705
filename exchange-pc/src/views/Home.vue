...
// 使用 computed 确保响应式更新
 const quickList = computed(() => [
   { icon: '/img/xyjk.png', label: localeStore.t('creditLoan') },
   { icon: '/img/yhkmm.png', label: localeStore.t('bankCardBuy') },
   { icon: '/img/lc.png', label: localeStore.t('financialManagement') },
 ])
...
// 处理快速入口点击
function handleQuickItemClick(label: string) {
  if (label === localeStore.t('bankCardBuy')) {
    router.push('/deposit')
  } else if (label === localeStore.t('creditLoan')) {
    router.push('/credit-loan')
  } else if (label === localeStore.t('financialManagement')) {
...
<template>
  <div class="home">
    <div class="card">
      ...
      <div class="user-row" v-if="auth.token">
        <div class="user-text">
          <span>{{ nickname || (auth.user && auth.user.email) || localeStore.t('user') }}&nbsp;&nbsp;{{ localeStore.t('vip') }}：0</span>
          <span class="uid">{{ localeStore.t('uid') }} {{ uid || (auth.user && auth.user.id) }}</span>
        </div>
      </div>
      <div class="search-row">
        <div class="search-bar" @click="router.push('/search')">
          <svg
            class="svg-icon ml-12 w-20 h-20 search-icon"
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <circle cx="11" cy="11" r="6" stroke="#222" stroke-width="1.6" />
            <line x1="15.5" y1="15.5" x2="20" y2="20" stroke="#222" stroke-width="1.6" stroke-linecap="round" />
          </svg>
          <span class="search-placeholder">{{ localeStore.t('searchSymbolContract') }}</span>
        </div>
      </div>
